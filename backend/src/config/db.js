...
import { jobsSeed } from '../modules/jobs/jobsSeed.js';
...
const AUTO_SEED_ON_START =
  process.env.DB_SEED_ON_START === 'true' ||
  (process.env.DB_SEED_ON_START == null && process.env.NODE_ENV !== 'production');
...
export async function runDatabaseMigrations() {
  await ensurePool();
  await runMigrations(pool, { schemaFile });
}
...
export async function initializeDatabase(options = {}) {
  const { seed = AUTO_SEED_ON_START, force = false } = options;

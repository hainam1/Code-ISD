...
function buildScheduledStartAt(interviewDate, interviewTime) {
  const hasValidDate = /^\d{4}-\d{2}-\d{2}$/.test(interviewDate);
  const hasValidTime = /^\d{2}:\d{2}(:\d{2})?$/.test(interviewTime);

  if (!hasValidDate || !hasValidTime) {
    return null;
  }

  const normalizedTime = interviewTime.length === 5 ? `${interviewTime}:00` : interviewTime;
  return `${interviewDate} ${normalizedTime}`;
}
...
router.get('/', authRequired, requireRoles('HR', 'MANAGEMENT'), async (_req, res) => {
  ...
  const interviews = rows.map((interview) => ({
    ...
    scheduledAt: toNullableIso(interview.scheduled_start_at),
...
router.post('/', authRequired, requireRoles('HR'), async (req, res) => {
  ...
  const result = await withTransaction(async (connection) => {
    ...
    const interview = {
      ...
      scheduledStartAt: buildScheduledStartAt(interviewDate, interviewTime),
      interviewDate,
      interviewTime,
      interviewLocation,
      result: null,
    ...
    await connection.query(
      `INSERT INTO interviews
        (id, application_id, candidate_id, job_id, scheduled_start_at, interview_date, interview_time, interview_location, result, comments, scheduled_by, evaluated_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        interview.id,
...
router.put('/:applicationId/evaluation', authRequired, requireRoles('HR'), async (req, res) => {
  ...
  const result = await withTransaction(async (connection) => {
    const [interviewRows] = await connection.query(
      `SELECT id, application_id, candidate_id, job_id, scheduled_start_at, interview_date, interview_time, interview_location, scheduled_by, created_at
       FROM interviews
    ...
    return {
      ...
      body: {
        ...
        interview: {
          ...
          scheduledAt: toNullableIso(interviewRow.scheduled_start_at),

...
const NO_APPLICATION_STATUS = 'No Application';
...
function mapCandidate(user, latestApplication) {
  ...
  return {
    ...
    status: latestApplication?.status || NO_APPLICATION_STATUS,
    cvFileName: cvPath ? cvPath.split('/').pop() : '',
    healthCertificateFileName: healthPath ? healthPath.split('/').pop() : '',
    cvFile: cvPath
      ? { fileName: latestApplication.cv_original_name || cvPath.split('/').pop(), mimeType: latestApplication.cv_mime_type || '', relativePath: cvPath }
      : null,
    healthFile: healthPath
      ? { fileName: latestApplication.health_original_name || healthPath.split('/').pop(), mimeType: latestApplication.health_mime_type || '', relativePath: healthPath }
      : null,
